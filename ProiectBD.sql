...
--Sa se afiseze in ordine descrescatoare mediile studentilor la teste care sunt inscrisi in clasa cu cei mai multi elevi din scoala.
with medii as
(select round(avg(calificativ), 2) medie, e.nr_matricol
from nota n, elev e
where n.nr_matricol = e.nr_matricol
and n.nume = 'evaluare'
group by e.nr_matricol)

select nume, prenume, medii.medie
from elev, medii 
where medii.nr_matricol = elev.nr_matricol
and id_clasa in  (select c.id_clasa
                  from elev e, clasa c
...
--Sa se afiseze pentru fiecare clasa olimpiadele la care au participat elevii inscrisi.
select o.disciplina, p.premiu, c.an, c.litera
from olimpiada o, clasa c, elev e, participa p
where o.cod_olimpiada = p.cod_olimpiada 
and e.nr_matricol = p.nr_matricol
and c.id_clasa = e.id_clasa
order by 2,3, 4, 1;

--Sa se afiseze pentru elevii cu numele Maftei media obtinuta la fiecare tip de examinare
select e.nume || ' ' || e.prenume Elev, nvl(round(avg(decode(n.nume, 'examen', calificativ)), 1), 0) Examen,
nvl(round(avg(decode(n.nume, 'teza', calificativ)), 2), 0) Teza,
nvl(round(avg(decode(n.nume, 'evaluare finala', calificativ)), 3), 0) Evaluare_finala,
nvl(round(avg(decode(n.nume, 'evaluare', calificativ)),4), 0) Evaluare,
nvl(round(avg(decode(n.nume, 'test', calificativ)), 5), 0) Test,
nvl(round(avg(decode(n.nume, 'activitate independenta', calificativ)), 6), 0) Activitate_independenta
from elev e, nota n
where e.nr_matricol = n.nr_matricol
and initcap(e.nume) = 'Maftei'
group by e.nume, e.prenume;

-- sa se afiseze numele elevilor pentru cei care venitul parintilor este egal cu cel mai mic venit din clasa al parintilor unui 
-- elev care primeste o bursa egala cu cea mai mica in categoria sa, doar daca nu sunt majori, au fost nascuti in prima jumatate
-- a anului
select e.nume, e.prenume, e.id_bursa
from elev e, clasa c
where to_char(sysdate, 'yyyy') - to_char(e.data_nasterii, 'yyyy') < 18
and extract(month from e.data_nasterii) < 7
and e.id_clasa = c.id_clasa
and e.venit_parinti = (select min(venit_parinti)
                       from elev aux
                       where aux.id_clasa = e.id_clasa
                       and aux.id_bursa in (select id_bursa
                                            from bursa b
                                            where b.suma = (select min(suma)
                                                            from bursa s)));

--Sa se determine pentru fiecare elev care sufera de o conditie medicala un alis format din prima litera a numelui, prima 
--litera a prenumelui si 1, iar pentru cei care sunt sanatosi un alias format din prima litera a numelui, prima 
--litera a prenumelui si 0.
select nume, prenume, 
case (conditie_medicala)
when 1 then concat(substr(nume, 0, 1), concat(substr(prenume, 0, 1), '1'))
when 0 then concat(substr(nume, 0, 1), concat(substr(prenume, 0, 1), '0'))
end "alias"
from elev;

--pentru fiecare profesor angajat inainte de anul 2012 care este repartizat la o incadrare sa i se mareasca salariul cu 250 de lei
update PROFESOR
set salariu = salariu + 250
where to_char(data_angajarii, 'yyyy') < 2012
and cod_profesor in (select cod_profesor 
                    from preda);

--toti elevii din clasa cu id-ul 100 sunt mutati in clasa cu id-ul 101 daca au bursa 705
update ELEV
set id_clasa = 101
where id_clasa = 100
and id_bursa in (select id_bursa
                from bursa 
                where suma = '705');

--sa se stearga din tabel toate olimpiadele la care nu participa niciun student
delete from olimpiada
where cod_olimpiada not in (select cod_olimpiada
                        from participa);

--Sa se afiseze numele elevilor, bursa pe care o primesc si banca la care au deschis un cont cu contul aferent (se vor afisa si
--cei care nu iau bursa sau care nu au un cont deschis) doar daca venitul parintilor este mai mare de 1000 de lei.
select e.nume, suma, cont_bancar, k.id_banca
from elev e, bursa b, cont c, banca k
where e.id_bursa = b.id_bursa (+)
and e.nr_matricol = c.nr_matricol (+)
and c.id_banca = k.id_banca (+)
and e.venit_parinti > 1000;

--Sa se obtina numerele matricole ale studentilor care au participat la toate olimpiadele de filologie
--(limba romana, limba engleza, limba franceza, etc.)
select distinct nr_matricol
from participa p1
where not exists
    (select 1
    from olimpiada o
    where o.disciplina like 'limba%'
    and not exists 
        (select 1
        from participa p2
        where o.cod_olimpiada = p2.cod_olimpiada
        and p2.nr_matricol = p1.nr_matricol));

--Sa se afiseze id-ul profesorilor care predau la toate clasele de a 8-a
select distinct cod_profesor
from preda p1
where not exists 
    (select 1
    from clasa c
    ...
    and not exists 
        (select 1
        from preda p2
        where c.id_clasa = p2.id_clasa
        and p2.cod_profesor = p1.cod_profesor));

--Afisati informatii despre elevii care au un cont deschis la banca ‘Banca Transilvania’
select e.*
from elev e, cont c, banca b
where c.nr_matricol = e.nr_matricol
and c.id_banca = b.id_banca
and b.nume = 'Banca Transilvania';

select e.*
from elev e, cont c, banca b
where b.nume = 'Banca Transilvania'
and c.nr_matricol = e.nr_matricol
and c.id_banca = b.id_banca;
